...
const int cServoChannel      = 5;                          // encoder pulses per motor revolution


// Variables
boolean motorsEnabled = true; 
boolean timeUp4sec = false;                                                   // motors enabled flag
...
unsigned long timerCount4sec = 0;                                                  // pushbutton debounce timer count
...
void loop()
{
   ...
   if ((currentMicros - previousMicros) >= 1000) {                             // enter when 1 ms has elapsed
      ...
      if (timerCount4sec > 4000) {                                             // if 2 seconds have elapsed
         timerCount4sec = 0;                                                   // reset 2 second timer count
         timeUp4sec = true;                                                    // indicate that 2 seconds have elapsed
      }
      ...
      switch(robotModeIndex) {
         case 0: // Robot stopped
            ...
            setMotor(0, 0, cIN1Chan[1], cIN2Chan[1]);                          // stop right motor                                           // clear right encoder
         ...
         case 1: // Run robot
            if (timeUp2sec) {                                                  // pause for 3 sec before running case 1 code
               ...
               if (motorsEnabled) {                                            // run motors only if enabled
                     ...
                     switch(driveIndex) {                                      // cycle through drive states
                        case 0: // Stop
                           ...
                           encoder[1].pos = 0;  
                           ledcWrite(cServoChannel, degreesToDutyCycle(25));         // stop right motor
                        ...
                        case 1: // Drive forward — motors spin in opposite directions as they are opposed by 180 degrees
                           ...
                           setMotor(1, driveSpeed-5, cIN1Chan[1], cIN2Chan[1]);
  
                           if(encoder[0].pos >= 10000){
                             encoder[0].pos = 0;                                                // clear left encoder
                        ...
                        case 2: // Drive backward — motors spin in opposite directions as they are opposed by 180 degrees
                           ...
                           setMotor(-1, driveSpeed-5, cIN1Chan[1], cIN2Chan[1]);
                             // right motor forward (opposite dir from right)
                             
                           if(encoder[0].pos <= -10000){ // right motor reverse (opposite dir from left)
                           driveIndex = 3;
                           encoder[0].pos = 0; 
                           setMotor(0, driveSpeed, cIN1Chan[0], cIN2Chan[0]);  // left motor forward
                           setMotor(0, driveSpeed, cIN1Chan[1], cIN2Chan[1]); 
                           ledcWrite(cServoChannel, degreesToDutyCycle(70));   
                           }
                                                             // next state: turn left
                           break;
               ...
               if(driveIndex == 6){
            robotModeIndex = 0;
            driveIndex = 0; // Reset driveIndex to 0
            }
